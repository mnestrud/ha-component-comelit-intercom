...
async def extract_token(host: str, password: str = "comelit", port: int = 8080) -> Optional[str]:
    """
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        try:
            ...
            async with session.post(f"{base_url}/do-login.html", data=login_data, headers=login_headers) as resp:
                if resp.status != 200:
            ...
            async with session.post(f"{base_url}/create-backup.html", headers=headers) as resp:
...
async def extract_token_from_backup(backup_data: bytes) -> Optional[str]:
                ...
                for root, dirs, files in os.walk(tmpdir):
                    for file in files:
                        if file == "users.cfg":
                            ...
                            matches = re.findall(r'9:4:"([a-f0-9]{32})"', content, re.IGNORECASE)
                            if matches:
                            ...
                            hex_matches = re.findall(r'\b([a-f0-9]{32})\b', content, re.IGNORECASE)
                            if hex_matches:
                                _LOGGER.info(f"Found {len(hex_matches)} potential tokens via hex pattern")
