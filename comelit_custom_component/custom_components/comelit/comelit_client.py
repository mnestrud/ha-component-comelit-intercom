    ...
    async def connect(self):
        ...
        try:
            ...
            if hasattr(e, 'errno') and e.errno == 65:
                self.logger.error(f"Cannot reach {self.host}:{self.port} - macOS errno 65 (No route to host)")
                self.logger.error("This is a known issue with Python 3.13 on macOS. The device may still be reachable.")
                # Try subprocess workaround if available
                if await self._test_nc_connection():
                    self.logger.warning("Device is reachable via nc but not Python socket. This is a Python bug.")
                    raise ConnectionError(f"Python socket connection failed due to macOS bug. Device is reachable via other methods.")
                else:
                    raise ConnectionError(f"Cannot reach {self.host}:{self.port}")
            else:
                self.logger.error(f"OS Error connecting to {self.host}:{self.port}: {e}")
                raise
    ...
    async def _test_nc_connection(self) -> bool:
        ...
        try:
            ...
            result = subprocess.run(
                ['nc', '-z', '-w', '2', self.host, str(self.port)],
                capture_output=True
            )
